...
1. Based on the selected concept, write a Veo video prompt (50-175 words):
   ...
   - SCRIPT LENGTH (CRITICAL):
     - For 16-second videos: The script MUST be exactly 35-40 words maximum.
     - For 8-second videos: The script MUST be exactly 15-20 words maximum.
     This strict length ensures it fits perfectly into the chosen video duration without getting cut off.
   - SYNC AUDIO TO VISUAL ACTION: Because the video and audio are generated by AI simultaneously, you must write the voiceover so its natural spoken pacing aligns with the visual sequence. For example, the first 1/3 of the script should match the "Setup" visual, the middle matches the "Action/Interaction", and the final words land perfectly on the "Payoff/Logo Reveal".
