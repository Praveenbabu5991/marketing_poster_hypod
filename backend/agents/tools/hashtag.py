...
def _retry_with_backoff(func, max_retries: int = 5, base_delay: float = 5.0):
    ...
    for attempt in range(max_retries):
        try:
            ...
            logger.warning("[HASHTAG] Attempt %d timed out", attempt + 1)
            ...
            if "api" in error_str and "key" in error_str:
                raise
            logger.warning("[HASHTAG] Attempt %d failed: %s", attempt + 1, str(e)[:200])
        if attempt < max_retries - 1:
            delay = base_delay * (2 ** attempt)
            logger.warning("[HASHTAG] Retrying in %.0fs...", delay)
            time.sleep(delay)
    raise last_error
