...
1. Based on the selected concept, write a detailed video prompt following the Veo 5-part formula:
   ...
   - SCRIPT LENGTH (CRITICAL): 
     - For 16-second videos: The script MUST be exactly 20-24 words maximum.
     - For 8-second videos: The script MUST be exactly 10-12 words maximum.
     This strict length ensures it fits perfectly into the chosen video duration with natural pacing, guaranteeing the voiceover finishes completely before the video ends.
   - SYNC AUDIO TO VISUAL ACTION: Because the video and audio are generated by AI simultaneously, you must write the voiceover so its natural spoken pacing aligns with the visual sequence. For example, the first 1/3 of the words should match the "Setup" visual, the next 1/3 match the "Action", and the final words land perfectly on the "Payoff/Logo Reveal".
